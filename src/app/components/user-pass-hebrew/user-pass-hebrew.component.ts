...
@Component({
  selector: 'app-user-pass-hebrew',
  templateUrl: './user-pass-hebrew.component.html',
  styleUrls: ['./user-pass-hebrew.component.css']
})
export class UserPassHebrewComponent implements OnInit {
  ...
  newMember() {
    ...
    if (this.Rout == 1)
      this.router.navigateByUrl("/NewMemberAccountHebrew/1");
    if (this.Rout == 5)
      this.router.navigateByUrl("/NewMemberAccountHebrew/5");
    if (this.Rout == 2)
      this.router.navigateByUrl("/NewMemberAccountHebrew/2");
    if (this.Rout == 3)
      this.router.navigateByUrl("/NewMemberAccountHebrew/3");
    if (this.Rout == 4)
      this.router.navigateByUrl("/NewMemberAccountHebrew/4");
    if (this.Rout == 6)
      this.router.navigateByUrl("/NewMemberAccountHebrew/6");
    if (this.Rout == 222)
      this.router.navigateByUrl("/NewMemberAccountHebrew/222");
    if (this.Rout == 2222)
      this.router.navigateByUrl("/NewMemberAccountHebrew/2222");
  ...
  SendCheckUserPassword() {
    ...
    this.serverService.SendCheckUserPassword(this.item).subscribe((events) => {
      ...
      if (this.true == true) {
        ...
        if (this.Rout == 3) {
          this.router.navigateByUrl("/CongressRegistrationSingleHebrew/2");
        }
        if (this.Rout == 7) {
        ...
        if (this.Rout == 6) {
          this.serverService.CheckIfInDraft().subscribe((events) => {
            if (events == 0) {
              this.router.navigateByUrl("/FirstPageHebrew");
            }
            if (events == 1) {
              this.router.navigateByUrl("/RegistrationOneEnglish");
            }
            if (events == 2) {
              this.router.navigateByUrl("/CongressRegistrationSingleHebrew");
            }
          });
  ...
  forgot() {
    // alert("e");
    var a;
    this.serverService.forgetPassHebrew(this.EmailToResetPassword).subscribe((val) => {
      a = val;
      if (a == 1)
        this.router.navigateByUrl("/ResetPassSuccessHebrew");
      else
        this.errPass = true;
